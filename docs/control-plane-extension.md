...
The GitHub Pages build does not use these server gates. It enables a clearly labeled browser-local simulation for committed processor-outflow throttle controls and persists only terminal demo values in local storage.
...
- `control_type`: semantic discriminator. Throttles have stricter topology validation than generic controls.
...
`control type + edge ID + dimensions + handler` → `service config target`.
...
Throttle controls intentionally cannot target a node or route: each throttle must map to one direct edge whose source node has `type: processor`. Other `control_type: generic` controls may use node or route targets when their semantics require it.
...
- Keep throttle controls per processor-outflow edge for high-specificity tuning and validation.
- Use route-level generic controls only when a non-throttle intent intentionally spans multiple edges.
